...
class ETABroadcastRequest(BaseModel):
    spot_id: str = Field(..., description="ID of the spot owner is returning to")
    minutes_remaining: int = Field(default=20, ge=5, le=120, description="Estimated minutes to arrival")
    host_name: str = Field(default="Ahmet Yılmaz", description="Owner name")
    note: str | None = Field(default=None, description="Optional message (e.g. Left office, traffic on highway)")
